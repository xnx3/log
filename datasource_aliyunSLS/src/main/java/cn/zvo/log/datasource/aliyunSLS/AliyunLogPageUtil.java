...
public class AliyunLogPageUtil {
	 ...
	 */
	public AliyunLogPageUtil(String endpoint, String accessKeyId, String accessKeySecret, String project) {
		this.aliyunLogUtil = new AliyunLogUtil(endpoint, accessKeyId, accessKeySecret, project);
	}
	 ...
	 * @throws LogException
	 */
	public ArrayList<QueriedLog> list(String logstore, String query, String topic, boolean divQuery, int startTime, int endTime, int everyPageNumber, int currentPage) throws LogException{
		if(endTime == 0){
...
//		}
		...
		//统计1000天内的符合条件的总数量
		int count = (int) aliyunLogUtil.queryCount(logstore, query, "", startTime, endTime);
		
//		page = new Page(count, everyPageNumber, request);
		page = new Page(count, everyPageNumber, currentPage);
		
		//最近三个月访问量统计
		ArrayList<QueriedLog> logList = aliyunLogUtil.queryList(logstore, query, "", startTime, endTime, page.getLimitStart(), page.getEveryNumber(), true);
		return logList;
	}
	 ...
	 * @throws LogException
	 */
	public JSONArray listForJSONArray(String logstore, String query, String topic, boolean divQuery, int startTime, int endTime, int everyPageNumber, int currentPage) throws LogException{
		//super.list(query, topic, divQuery, startTime, endTime, everyPageNumber, request)
		ArrayList<QueriedLog> logList = list(logstore, query, topic, divQuery, startTime, endTime, everyPageNumber, currentPage);
	 ...
	 * @throws LogException
	 */
	public JSONArray list(String logstore, String query, String topic, boolean divQuery, int everyPageNumber, int currentPage) throws LogException{
		return listForJSONArray(logstore, query, topic, divQuery, 0, 0, everyPageNumber, currentPage);
	}
